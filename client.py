...
def serverListen(serverSocket):
	while True:
		msg = serverSocket.recv(2048).decode("utf-8")
		if msg == "/viewRequests":
			serverSocket.send(bytes(".","utf-8"))
			response = serverSocket.recv(2048).decode("utf-8")
			...
			if response == "/proceed":
				...
				print(serverSocket.recv(2048).decode("utf-8"))
			else:
				print(response)
		elif msg == "/disconnect":
			serverSocket.send(bytes(".","utf-8"))
			...
			filename = serverSocket.recv(2048).decode("utf-8")
...
def main():
	...
	state["username"] = input("Welcome to Chat! Please enter your username: ")
